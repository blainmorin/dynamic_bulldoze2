...
regressors = c("appt_pct", "expert_pct", "advs_pct", "b18_dng_r")
# names(fed)
...
dff = df %>%
  ...
  mutate(appt_and_advs = appt_pct + advs_pct) %>%
  mutate_at(c("appt_and_advs", "expert_pct", "b18_dng_r"), scale) %>%
  drop_na() 

regressors = c("appt_and_advs", "expert_pct", "b18_dng_r")
